...
;;; Commentary:
;;
;; This program is modified from the  original `modus-themes-exporter.el'
;; by Protesilaos Stavrou for use with that authors other excellent theme
;; package, Ef Themes.

;; EXPERIMENTAL AND ONLY INTENDED FOR ADVANCED USERS
;;
;; Helper functions to produce a port of the currently
;; enabled Ef theme for various terminal emulators or text editors
;; outside the Emacs milieu.
;;
...
;; approximate the surface-level aesthetics of the Ef themes: a real
;; theme requires attention to detail; a requirement that no generic
;; template can satisfy.
;;
;; Consult the doc string of `ef-themes-exporter-export' for how to
;; use this library.
;;
;; Find the orginal Authors blog post introducing
;; `modus-themes-exporter.el' here:
;; "Introducing the modus themes exporter (Emacs library)"
;; <https://protesilaos.com/codelog/2021-02-22-ef-themes-exporter/>
...
(defun ef-themes-exporter--current-theme ()
  "Return current ef theme."
  (car
   (seq-filter
    (lambda (theme)
      (string-match-p "^ef" (symbol-name theme)))
    custom-enabled-themes)))

(defvar ef-themes-exporter-templates-alist
  '(("alacritty"        . ef-themes-exporter-alacritty)
    ("foot"             . ef-themes-exporter-foot)
    ("iterm2"           . ef-themes-exporter-iterm2)
    ("urxvt"            . ef-themes-exporter-urxvt)
    ("vim"              . ef-themes-exporter-vim)
    ("windows-terminal" . ef-themes-exporter-windows-terminal)
    ("xcolors"          . ef-themes-exporter-xcolors)
    ("xfce"             . ef-themes-exporter-xfce)
    ("xterm"            . ef-themes-exporter-xterm)))

;;;; Generic Xcolors template

(defun ef-themes-exporter-xcolors ()
  "Template for generic Xcolors."
  (ef-themes-with-colors
    (let ((theme-name (format "%s" (ef-themes-exporter--current-theme))))
      (with-temp-buffer
        (concat
         "! Theme: " theme-name "\n"
         "! Description: Generic Xcolors port of " theme-name " (Ef themes for Emacs)" "\n"
...
(defun ef-themes-exporter-xterm ()
  "Template for XTerm."
  (ef-themes-with-colors
    (let ((theme-name (format "%s" (ef-themes-exporter--current-theme))))
      (with-temp-buffer
        (concat
         "! Theme: " theme-name "\n"
         "! Description: XTerm port of " theme-name " (Ef themes for Emacs)" "\n"
         "! Author: Protesilaos Stavrou, <https://protesilaos.com>" "\n"
...
(defun ef-themes-exporter-alacritty ()
  ...
  (ef-themes-with-colors
    (let ((theme-name (format "%s" (ef-themes-exporter--current-theme))))
      (with-temp-buffer
        (concat
         "# Theme: " theme-name "\n"
         "# Description: Alacritty port of " theme-name " (Ef themes for Emacs)" "\n"
         "# Author: Protesilaos Stavrou, <https://protesilaos.com>" "\n"
         "colors:" "\n"
         "  primary:" "\n"
         "    background: '" bg-main "'" "\n"
    ...
    (let ((theme-name (format "%s" (ef-themes-exporter--current-theme))))
      (with-temp-buffer
        (string-replace
         ...
         (concat
          "# Theme: " theme-name "\n"
          "# Description: Foot port of " theme-name " (Ef themes for Emacs)" "\n"
          "# Author: Protesilaos Stavrou, <https://protesilaos.com>" "\n"
          "[colors]" "\n"
          "alpha=1.00" "\n"
          "background=" bg-main "\n"
...
(defun ef-themes-exporter-urxvt ()
  ...
  (ef-themes-with-colors
    (let ((theme-name (format "%s" (ef-themes-exporter--current-theme))))
      (with-temp-buffer
        (concat
         "! Theme: " theme-name "\n"
         "! Description: URxvt port of " theme-name " (Ef themes for Emacs)" "\n"
         "! Author: Protesilaos Stavrou, <https://protesilaos.com>" "\n"
...
(defun ef-themes-exporter-windows-terminal ()
  ...
  (ef-themes-with-colors
    (let ((theme-name (format "%s" (ef-themes-exporter--current-theme))))
      (with-temp-buffer
        (concat
         "// Theme: " theme-name "\n"
         "// Description: Windows Terminal port of " theme-name " (Ef themes for Emacs)" "\n"
         "// Author: Protesilaos Stavrou, <https://protesilaos.com>" "\n"
         "\"name\": \"" theme-name "\",\n"
         "\"background\": \"" bg-main "\",\n"
         "\"foreground\": \"" fg-main "\",\n"
...
;; templates.
(defun ef-themes-exporter-vim ()
  "Template for Vim."
  (ef-themes-with-colors
    (let* ((theme-name (format "%s" (ef-themes-exporter--current-theme)))
           (theme-name-underscore (string-replace "-" "_" theme-name))
           (variant (if (string= theme-name "ef-operandi") "light" "dark"))
...
"\" Description: Vim port of " theme-name " (Ef themes for Emacs)" "\n"
"\" Author: Protesilaos Stavrou, <https://protesilaos.com>" "\n"
"\" Meta: Created with the ef-themes-exporter.el library" "\n"
...
"\" General" "\n"
"\" -------" "\n"
"if exists('g:ef_themes_enforce_background_color')" "\n"
...
(defun ef-themes-exporter--iterm2-color-component (color component)
...
(defun ef-themes-exporter--iterm2-color-name (name)
  "Generate a DOM key entry for the specified NAME."
  `(key nil ,name))

(defun ef-themes-exporter--iterm2-color-dict (color &optional alpha)
...
The COLOR is a string in a form of #RRGGBB.  The ALPHA is a number."
  `(dict nil
         ...
         (real nil ,(ef-themes-exporter--iterm2-color-component color 'blue))
         (key nil "Color Space")
         (string nil "sRGB")
         (key nil "Green Component")
         (real nil ,(ef-themes-exporter--iterm2-color-component color 'green))
         (key nil "Red Component")
         (real nil ,(ef-themes-exporter--iterm2-color-component color 'red))))

(defun ef-themes-exporter--iterm2-dict (colors-defs)
     ...
     `(dict nil
            ,@(apply 'cl-concatenate
                     ...
                     (mapcar (lambda (color-name)
                               (list (ef-themes-exporter--iterm2-color-name (car color-name))
                                     (apply 'ef-themes-exporter--iterm2-color-dict (pcase (cdr color-name)
...
(defun ef-themes-exporter-iterm2 ()
  "Template for iTerm2."
  (ef-themes-with-colors
    (let ((theme-name (format "%s" (ef-themes-exporter--current-theme))))
      (with-temp-buffer
        (concat
         ...
         "<!-- Theme: " theme-name "-->\n"
         "<!-- Description: iTerm2 port of " theme-name " (Ef themes for Emacs)" "-->\n"
         "<!-- Author: Przemek Kryger" "-->\n"
         "<plist version=\"1.0\">\n"
         (ef-themes-exporter--iterm2-dict
          `(("Ansi 0 Color" . "#000000")
            ("Ansi 1 Color" . ,red)
            ("Ansi 2 Color" . ,green)
            ("Ansi 3 Color" . ,yellow)
...
(defvar ef-themes-exporter-template-hist '()
  "History of inputs for templates.")

(defun ef-themes-exporter--export-prompt (apps)
  "Helper for `ef-themes-exporter-export' to select among APPS."
  (completing-read "Select template: " apps nil t nil 'ef-themes-exporter-template-hist))

;;;###autoload
(defun ef-themes-exporter-export (template &optional file no-visit)
  "Export current Ef theme using TEMPLATE.
...
of a cons cell in `ef-themes-exporter-templates-alist'.
...
file."
  (interactive
   (list
    (ef-themes-exporter--export-prompt
     (mapcar #'car ef-themes-exporter-templates-alist))
    current-prefix-arg))
  (let* ((current-theme (ef-themes-exporter--current-theme))
         (fn (funcall (cdr (assoc template ef-themes-exporter-templates-alist))))
         (path (when file
                 (if (stringp file)
                     file
                   (expand-file-name
...
(provide 'ef-themes-exporter)

;;; ef-themes-exporter.el ends here
